...
interface NutritionalRiskFormProps {
  ...
  onSave: (record: NutritionalRiskRecord) => void;
  onCancel: () => void;
}

const NutritionalRiskForm = ({ initialRecord, onSave, onCancel }: NutritionalRiskFormProps) => {
  const [record, setRecord] = useState<Partial<NutritionalRiskRecord>>({
    id: '',
    date: new Date().toISOString(),
    professional: 'Dr García Martínez',
    items: nutritionalRiskItems.map(item => ({ itemId: item.id, value: 0 })),
    score: 0
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const completeRecord: NutritionalRiskRecord = {
  ...
  const renderNutritionalRiskItem = (item: NutritionalRiskItem) => {
  ...
  const currentScoreDescription = nutritionalRiskScoreDescriptions.find(
    desc => (record.score ?? 0) >= desc.min && (record.score ?? 0) <= desc.max
  );

  const scoreColor = getNutritionalRiskResultColor(record.score || 0);

  return (
    <form className="barthel-form" onSubmit={handleSubmit}>
      <div className="form-header">
        <h2>{initialRecord ? 'Editar Registro de Valoración Nutricional' : 'Nuevo Registro de Valoración Nutricional'}</h2>
        ...
        <div className="score-display" style={{ borderLeft: `4px solid ${scoreColor}` }}>
          <div className="score-info">
            ...
            <span className="score-value" style={{ color: scoreColor }}>{record.score}/9</span>
          </div>
          <div className="score-description">
            <span className="score-result">{getNutritionalRiskResultText(record.score || 0)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="barthel-scale-info">
        <h3>Escala de Valoración STAMP</h3>
        ...
        <table className="barthel-scale-table">
          ...
          <tbody>
            {nutritionalRiskScoreDescriptions.map((desc, index) => (
              <tr
                key={index}
...
export default NutritionalRiskForm;
